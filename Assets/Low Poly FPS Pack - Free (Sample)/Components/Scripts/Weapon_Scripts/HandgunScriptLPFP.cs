...
public class HandgunScriptLPFP : MonoBehaviour {
    ...
    public float recoilLoop;
    public float recoilSpeed;
    //Check if out of ammo
    private bool outOfAmmo;
    //Check if out of Backup Ammo
    private bool outOfBackupAmmo;

    private bool isfire = false;

    private bool isfrontSightDisplay = true;
    ...
    private void Awake () 
	{
        ...
        loopSize = loop.GetComponent<RectTransform>().sizeDelta;
	...
	private void Start () {
        ...
        for (int i = 0; i < AmmoCtr.Instance.armList.Length; i++)
        {
            if (AmmoCtr.Instance.armList[i].armName == weaponName)
                armNumber = i;
        }
    ...
    private void Update()
    {
        lifebar.fillAmount = 1 * (player.GetComponent<Player>().health * 0.01f);
        if (!gameCtr.GetComponent<GameCtr>().settingFlag&& !gameCtr.GetComponent<GameCtr>().deathFlag&& !gameCtr.GetComponent<GameCtr>().winFalg)
        {           
            //Aiming
            //Toggle camera FOV when right click is held down
            if (isfire)
            {
                loop.GetComponent<RectTransform>().sizeDelta = Vector2.Lerp(loopSize, loopSize * recoilLoop, 1f);
            }
            else
            {
                loop.GetComponent<RectTransform>().sizeDelta = Vector2.Lerp(loop.GetComponent<RectTransform>().sizeDelta, loopSize, recoilSpeed * Time.deltaTime);
            }
            isfire = false;
            ...
            currentAmmoText.text = AmmoCtr.Instance.armList[armNumber].magazineClipAmmo.ToString();

            //Set total ammo text from total ammo int
            totalAmmoText.text = AmmoCtr.Instance.armList[armNumber].backupAmmo.ToString();
            ...
            if (AmmoCtr.Instance.armList[armNumber].backupAmmo == 0)
            {
                outOfBackupAmmo = true;
            }
            else
            {
                outOfBackupAmmo = false;
            }

            //If out of ammo
            if (AmmoCtr.Instance.armList[armNumber].magazineClipAmmo == 0)
            {
                ...
                outOfAmmo = true;
                //Auto reload if true
                if (autoReload == true && !isReloading && !outOfBackupAmmo)
                {
            ...
            if (Input.GetMouseButtonDown(0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning)
            {
                ...
                AmmoCtr.Instance.armList[armNumber].magazineClipAmmo -= 1;
            ...
            //Reload 
            if (Input.GetKeyDown(KeyCode.R) && !isReloading && !isInspecting && !outOfBackupAmmo && AmmoCtr.Instance.armList[armNumber].magazineClipAmmo != AmmoCtr.Instance.armList[armNumber].magazineClip)
            {
                //Reload
                Reload();
            ...
            if (isfrontSightDisplay && !isInspecting)
            ...
            if (gameCtr.GetComponent<GameCtr>().caissonFlag == true)
            {
                supplyText.gameObject.SetActive(true);
                if (Input.GetKey(KeyCode.C) && AmmoCtr.Instance.armList[armNumber].backupAmmo < AmmoCtr.Instance.armList[armNumber].maxAmmo)
                {
                    if (supplyProgress >= 100)
                    {
                        AmmoCtr.Instance.ammoSupply = true;
                        supplybar.SetActive(false);
                        supplyProgress = 0;
                    }
                    else
                    {
                        supplybar.SetActive(true);
                        supplyProgress += supplySpeed * Time.deltaTime;
                        nowSupplybar.fillAmount = 1 * (supplyProgress * 0.01f);
                    }
                }
                else
                {
                    AmmoCtr.Instance.ammoSupply = false;
                    supplybar.SetActive(false);
                    supplyProgress = 0;
                }
            }
    ...
    void ItemDetect()
    {
        ...
        bool hit = Physics.Raycast(origin, transform.forward, out raycastHit, detectDistance, ~layerMask, QueryTriggerInteraction.Ignore);
        Debug.DrawRay(origin, transform.forward * detectDistance, Color.red);
        if (hit)
        {
            if (raycastHit.transform.tag == "Caisson")
